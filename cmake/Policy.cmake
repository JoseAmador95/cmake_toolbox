...
function(_policy_check_newold VALUE OUTVAR)
...
function(policy_register)
    ...
    set(multiValueArgs)
    cmake_parse_arguments(POLICY_REG "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if(NOT POLICY_REG_NAME)
        message(FATAL_ERROR "POLICY: policy_register requires NAME <policy_name>")
    endif()
    if(NOT POLICY_REG_DESCRIPTION)
        message(FATAL_ERROR "POLICY: policy_register requires DESCRIPTION <description>")
    endif()
    if(NOT POLICY_REG_DEFAULT)
        message(FATAL_ERROR "POLICY: policy_register requires DEFAULT <NEW|OLD>")
    endif()
    if(NOT POLICY_REG_INTRODUCED_VERSION)
        message(FATAL_ERROR "POLICY: policy_register requires INTRODUCED_VERSION <version>")
    endif()

    _policy_check_newold("${POLICY_REG_DEFAULT}" _checked_default)
    _policy_registry_get(_policy_registry)
    foreach(_entry ${_policy_registry})
        if(NOT _entry STREQUAL "")
            ...
            if(_existing STREQUAL "${POLICY_REG_NAME}")
                message(FATAL_ERROR "POLICY: Already registered: ${POLICY_REG_NAME}")
            endif()
        endif()
    endforeach()
    _policy_registry_append("'${POLICY_REG_NAME}'|'${POLICY_REG_DESCRIPTION}'|'${POLICY_REG_DEFAULT}'|'${POLICY_REG_INTRODUCED_VERSION}'")
endfunction()
...
function(policy_set)
    ...
    set(multiValueArgs)
    cmake_parse_arguments(POLICY_SET "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if(NOT POLICY_SET_POLICY)
        message(FATAL_ERROR "POLICY: policy_set requires POLICY <policy_name>")
    endif()
    if(NOT POLICY_SET_VALUE)
        message(FATAL_ERROR "POLICY: policy_set requires VALUE <NEW|OLD>")
    endif()

    _policy_find("${POLICY_SET_POLICY}" _idx)
    if(_idx LESS 0)
        message(FATAL_ERROR "POLICY: ${POLICY_SET_POLICY} not registered")
    endif()
    _policy_check_newold("${POLICY_SET_VALUE}" _okval)
    _policy_write("${POLICY_SET_POLICY}" "${_okval}")
endfunction()
...
function(policy_get)
    ...
    set(multiValueArgs)
    cmake_parse_arguments(POLICY_GET "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if(NOT POLICY_GET_POLICY)
        message(FATAL_ERROR "POLICY: policy_get requires POLICY <policy_name>")
    endif()
    if(NOT POLICY_GET_OUTVAR)
        message(FATAL_ERROR "POLICY: policy_get requires OUTVAR <output_variable>")
    endif()

    _policy_find("${POLICY_GET_POLICY}" _idx)
    if(_idx LESS 0)
        message(FATAL_ERROR "POLICY: ${POLICY_GET_POLICY} not registered")
    endif()
    _policy_registry_get(_policy_registry)
    list(GET _policy_registry ${_idx} _entry)
    if("${_entry}" STREQUAL "")
        message(FATAL_ERROR "POLICY: Registry is corrupted or empty for ${POLICY_GET_POLICY}")
    endif()
    _policy_record_unpack("${_entry}" _fields)
    if(NOT _fields)
        message(FATAL_ERROR "POLICY: Policy entry malformed for ${POLICY_GET_POLICY}: ${_entry}")
    endif()
    list(LENGTH _fields _field_len)
    if(_field_len LESS 3)
        message(FATAL_ERROR "POLICY: Policy registry entry too short for ${POLICY_GET_POLICY} (fields: ${_fields})")
    endif()
    list(GET _fields 2 _def)
    _policy_read(${POLICY_GET_POLICY} _val)
    if(NOT _val STREQUAL "")
        _policy_check_newold("${_val}" _val)
        set(${POLICY_GET_OUTVAR} "${_val}" PARENT_SCOPE)
    else()
        message(NOTICE "POLICY: '${POLICY_GET_POLICY}' not set, using default: ${_def}")
        set(${POLICY_GET_OUTVAR} "${_def}" PARENT_SCOPE)
    endif()
endfunction()
...
function(policy_version)
    ...
    set(multiValueArgs)
    cmake_parse_arguments(POLICY_VERSION "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if(NOT POLICY_VERSION_MINIMUM)
        message(FATAL_ERROR "POLICY: policy_version requires MINIMUM <min_version> (usage: MINIMUM <ver> [MAXIMUM <ver>])")
    endif()

    set(min_version "${POLICY_VERSION_MINIMUM}")
    set(do_max 0)
    if(POLICY_VERSION_MAXIMUM)
        set(max_version "${POLICY_VERSION_MAXIMUM}")
        set(do_max 1)
    endif()
