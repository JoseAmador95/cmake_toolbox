...
function(_policy_record_unpack RECORD OUTVAR)
    set(_record "${RECORD}")
    string(REPLACE "\\|" "___ESCAPED_PIPE___" _record "${_record}")
    string(REPLACE "|" ";" _fields "${_record}")

    set(_is_legacy FALSE)
    list(LENGTH _fields _field_len)
    if(_field_len EQUAL 7)
        set(_all_quoted TRUE)
        foreach(_field ${_fields})
            if(NOT _field MATCHES "^'.*'$")
                set(_all_quoted FALSE)
                break()
            endif()
        endforeach()
        if(_all_quoted)
