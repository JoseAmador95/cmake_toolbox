...
if(Cppcheck_FOUND)
    ...
    if(NOT ("${cppcheck_cmd}" MATCHES "-i;"))
        message(STATUS "FAIL: No -i flags in command: ${cppcheck_cmd}")
        set(test_failed TRUE)
    else()
        if(NOT ("${cppcheck_cmd}" MATCHES "-i;[^;]*build"))
            message(STATUS "FAIL: Pattern '^build/.*' not found in -i flags: ${cppcheck_cmd}")
            set(test_failed TRUE)
        else()
            message(STATUS "PASS: Pattern '^build/.*' detected")
        endif()

        if(NOT ("${cppcheck_cmd}" MATCHES "-i;[^;]*third_party"))
            message(STATUS "FAIL: Pattern '.*third_party.*' not found in -i flags: ${cppcheck_cmd}")
            set(test_failed TRUE)
        else()
            message(STATUS "PASS: Pattern '.*third_party.*' detected")
        endif()

        if(NOT ("${cppcheck_cmd}" MATCHES "-i;[^;]*generated"))
            message(STATUS "FAIL: Pattern '*/generated/*' not found in -i flags: ${cppcheck_cmd}")
            set(test_failed TRUE)
        else()
