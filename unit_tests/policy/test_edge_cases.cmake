...
set(LONG_NAME
    "EXTREMELY_LONG_POLICY_NAME_THAT_TESTS_THE_LIMITS_OF_THE_POLICY_SYSTEM_ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789"
)
set(LONG_DESC
    "This is an extremely long description that tests how the policy system handles very long text fields. It includes multiple sentences and should test the robustness of the string handling mechanisms in the policy system."
)

Policy_Register(NAME ${LONG_NAME} DESCRIPTION "${LONG_DESC}" DEFAULT OLD INTRODUCED_VERSION 1.0.0)
...
Policy_Register(
    NAME VER_COMPLEX_001
    DESCRIPTION "Complex version 1"
    DEFAULT OLD
    INTRODUCED_VERSION 10.20.30
)
Policy_Register(
    NAME VER_COMPLEX_002
    DESCRIPTION "Complex version 2"
    DEFAULT OLD
    INTRODUCED_VERSION 1.0
)
Policy_Register(
    NAME VER_COMPLEX_003
    DESCRIPTION "Complex version 3"
    DEFAULT OLD
...
if(NOT complex1 STREQUAL "OLD")
    message(
        SEND_ERROR
        "VER_COMPLEX_001 should be OLD (10.20.30 > 5.0.0, so not set to NEW), got: ${complex1}"
    )
    math(EXPR ERROR_COUNT "${ERROR_COUNT} + 1")
endif()
...
if(NOT complex2 STREQUAL "NEW")
    message(
        SEND_ERROR
        "VER_COMPLEX_002 should be NEW (5.0.0 >= 1.0, so set to NEW), got: ${complex2}"
    )
    math(EXPR ERROR_COUNT "${ERROR_COUNT} + 1")
endif()
...
if(NOT complex3 STREQUAL "NEW")
    message(
        SEND_ERROR
        "VER_COMPLEX_003 should be NEW (5.0.0 >= 0.1.0, so set to NEW), got: ${complex3}"
    )
    math(EXPR ERROR_COUNT "${ERROR_COUNT} + 1")
endif()
...
Policy_Register(
    NAME COMPLEX_WARNING
    DESCRIPTION "Policy with complex warning"
    DEFAULT OLD
    INTRODUCED_VERSION 2.0
    WARNING
        "Warning with: | pipes || double pipes ||| triple pipes
...
# Policy_Version with MINIMUM+MAXIMUM range semantics:
# - Policies with introduced_version in [MINIMUM, MAXIMUM] are set to NEW
# - Policies with introduced_version > MAXIMUM are set to OLD
# For MINIMUM 10.0 MAXIMUM 15.0:
# - STRESS_10-15 have introduced_version in [10.0, 15.0] -> NEW
# - STRESS_5 was explicitly set to NEW earlier and is outside the range -> preserved as NEW
# Total: STRESS_5, STRESS_10-15 = 7 policies NEW
set(expected_new_count 7)
if(NOT actual_new_count EQUAL expected_new_count)
    message(SEND_ERROR "Expected ${expected_new_count} policies to be NEW, got ${actual_new_count}")
    math(EXPR ERROR_COUNT "${ERROR_COUNT} + 1")
endif()
...
Policy_Register(NAME MINIMAL_TEST DESCRIPTION "M" DEFAULT OLD INTRODUCED_VERSION 1.0 WARNING "")
